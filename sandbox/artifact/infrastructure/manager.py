...
class ArtifactManager:
    ...
    def get_for_owner_scope(
        ...
        *,
        ...
        """Resolve an Artifact by its durable owner, independent of source session.

        This is intentionally separate from ``get_for_owner``. Downloads stay
        bound to the source conversation/agent session, while a cross-session
        import is an explicit owner-authorized operation that may read the
        ...
        if self._formal.enabled:
            formal = self._formal.get(
                artifact_id,
                OwnerScope(org_id=org, user_id=user),
            )
            if formal is None or str(formal.status or "").lower() != "ready":
                return None
            response = self._formal_to_response(formal, session_id=session_id)
            with self._lock:
                self._artifacts[artifact_id] = {
                    "artifact_id": formal.artifact_id,
                    "session_id": session_id,
        ...
        # Development/offline mode has no restart-safe authority. Permit only
        # a live cache entry carrying an exact owner stamp; unowned legacy
        # artifacts cannot cross sessions.
        with self._lock:
            entry = self._artifacts.get(artifact_id)
            if entry is None:
                return None
            if str(entry.get("org_id") or "") != org:
                return None
            if str(entry.get("user_id") or "") != user:
                return None
            if str(entry.get("status") or "ready").lower() != "ready":
                return None
            return self._to_response(entry)
    ...
    def _resolve_snapshot(
        ...
        *,
        ...
        """Resolve and verify the immutable snapshot behind an authorized row."""
        ...
        if ident_dict and sha:
            ...
            if int(st.st_size) != int(artifact.size or st.st_size):
                raise ArtifactError(
                    "artifact_identity_mismatch",
                    "Artifact snapshot size mismatch",
                    status=409,
                )
        return artifact, snap, identity

    def resolve_owner_snapshot(
        self,
        *,
        artifact_id: str,
        ...
        """Resolve an immutable source snapshot for an explicit import."""
        artifact = self.get_for_owner_scope(
            artifact_id,
        ...
        if int(resolved[2].st_size) != int(artifact.size):
            raise ArtifactError(
                "artifact_identity_mismatch",
                "Artifact snapshot size does not match formal metadata",
                status=409,
            )
        return resolved

    def source_provenance(self, artifact_id: str) -> dict[str, str | None]:
        """Return immutable source bindings for audit after authorization."""
        with self._lock:
            entry = self._artifacts.get(artifact_id) or {}
            return {
                "conversation_id": (
