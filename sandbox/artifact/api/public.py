...
from sandbox.artifact.application.facade import artifact_facade
...
def _trusted_owner(session, request: Request) -> tuple[str | None, str | None]:
    """Resolve owner identity from the authenticated hop and owned session."""
    from sandbox.security.ownership import (
        session_organization_id,
        session_owner_user_id,
    )
    ...
    return (
        str(org).strip() if org else None,
        str(user).strip() if user else None,
    )
...
def list_artifacts(session_id: str, request: Request):
    ...
    org, uid = _trusted_owner(session, request)
    uid = uid or (str(bind["user_id"]) if bind.get("user_id") else None)
    artifacts = artifact_facade.list(
        session_id,
        org_id=org,
...
@router.post("/imports", response_model=ArtifactImportResponse, status_code=201)
def import_artifact(
    session_id: str,
    body: ArtifactImportRequest,
    request: Request,
):
    """Import an owner-scoped Artifact as a target-workspace input file."""
    session = _require_session(session_id, request)
    context = SandboxExecutionContext.from_session(session)
    bind = _session_binding(session)
    org, user = _trusted_owner(session, request)
    if not org or not user:
        raise HTTPException(status_code=401, detail="Artifact owner unavailable")
    try:
        return artifact_facade.import_to_workspace(
            ...
            target_conversation_id=(
                str(bind["conversation_id"])
                if bind.get("conversation_id")
                else None
            ),
            target_agent_session_id=(
                str(bind["agent_session_id"])
                if bind.get("agent_session_id")
                else None
            ),
        )
    except ArtifactError as exc:
...
async def download_artifact(session_id: str, artifact_id: str, request: Request):
    ...
    try:
        art, snap_path, identity = await asyncio.to_thread(
            artifact_facade.resolve_download,
            session_id=session_id,
