...
describe('attachment drafts', () => {
  it('createAttachmentDraft starts queued with unique localId and idempotency key', () => {
    ...
    const b = createAttachmentDraft(fakeFile('a.txt')); // same display name
  ...
  it('validateNewAttachments enforces count and size limits', () => {
    ...
    const existing = [
      { ...createAttachmentDraft(fakeFile('a.txt', 50)), status: 'uploaded', path: 'p' },
    ];
    ...
    assert.equal(tooMany.code, 'turn_attachment_limit');
    // single file
    const big = validateNewAttachments([], [fakeFile('big.txt', 101)], limits);
    assert.equal(big.ok, false);
    assert.equal(big.code, 'attachment_too_large');
    // turn total
    const turn = validateNewAttachments(
      [{ ...createAttachmentDraft(fakeFile('a.txt', 100)), status: 'uploaded', path: 'p' }],
      [fakeFile('b.txt', 60)],
      limits,
    );
  ...
  it('validateNewAttachments denies disallowed extensions (layered whitelist)', () => {
    ...
    assert.equal(denied.code, 'attachment_type_denied');
  });
  ...
  it('buildUserTurnWithAttachments injects manifest paths (no auto-send side effects)', () => {
    ...
    assert.match(msg.content[0].text, /Please review/);
    assert.match(msg.content[0].text, /\[Attachments\]/);
    assert.match(msg.content[0].text, /uploads\/att_a\/a\.txt/);
    assert.match(msg.content[0].text, /uploads\/att_b\/b\.md/);
  ...
  it('selecting files never implies auto-send (state stays non-streaming)', () => {
    // Pure state check: adding drafts does not start a stream / does not add user messages
    let s = createState(INITIAL);
