...
async function getJson(path: string, label: string): Promise<unknown> {
  const resp = await fetch(`/api/admin/runs${path}`, { headers: authHeaders() });
  if (!resp.ok) {
...
export async function listAdminRuns(f: AdminRunFilters = {}): Promise<AdminRunPage> {
  ...
  return parseApi(AdminRunPageSchema, await getJson(`?${q}`, 'List runs'), 'admin runs');
}
...
export async function getAdminRunStats(dayStart: Date): Promise<AdminRunStats> {
  ...
  return parseApi(AdminRunStatsSchema, await getJson(`/stats?${q}`, 'Run stats'), 'admin run stats');
}

export async function getAdminRun(runId: string): Promise<AdminRun> {
  return parseApi(AdminRunSchema, await getJson(`/${encodeURIComponent(runId)}`, 'Get run'), 'admin run');
}

export async function listAdminRunEvents(runId: string): Promise<PersistedAgentEvent[]> {
  const body = await getJson(`/${encodeURIComponent(runId)}/events`, 'Run events');
...
export async function listAdminRunTools(runId: string): Promise<ToolExecutionSnapshot[]> {
  const body = await getJson(`/${encodeURIComponent(runId)}/tools`, 'Run tools');
