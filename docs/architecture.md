...
│    workspace_id · relative paths · iptables · ulimit     │
└──────────────────────────────────────────────────────────┘
```
...
### 3. Session-owned workspace 隔离（相对路径 + `workspace_id`）

每个 Agent/Sandbox session 绑定 **唯一工作区**（1 session → 1 workspace）：
- 公共协议只暴露 opaque **`workspace_id`**；工具/文件/Artifact 路径相对 Session 根
- 内部物理根仅存于 service/repository，不进入 API、SSE、模型上下文或活跃文档示例
- Skills：共享 skill 根在 workspace 外；发行基线 **零内置 Skill package**。默认只读；`SKILLS_MODE=development` 时仅 `skill_install` / `skill_edit` / `skill_reload` 可写
- 串行执行（同一会话内命令排队，防止竞态）
- 30 分钟 TTL 自动清理
