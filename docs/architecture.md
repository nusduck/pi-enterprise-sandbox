...
- Skills：`/home/sandbox/skill`（共享，不进 workspace；默认只读。`SKILLS_MODE=development` 时 Agent 侧可写，仅 `skill_install` / `skill_edit` / `skill_reload` 专用工具可变更；Sandbox 执行侧始终只读挂载）
- 串行执行（同一会话内命令排队，防止竞态）
- 30 分钟 TTL 自动清理
...
| Agent SDK | 独立 Node Agent 服务（`@earendil-works/pi-coding-agent` 精确锁定） |
