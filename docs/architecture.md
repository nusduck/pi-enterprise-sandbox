...
| **API Server** | `pi-enterprise-api` | Node.js + pi-coding-agent SDK | BFF（CORS/auth/SSE）；默认 Node 编排，可选 `AGENT_RUNTIME=python` 代理到 Sandbox agent |
| **Sandbox** | `pi-enterprise-sandbox` | Python 3.11 + FastAPI | 安全命令执行、文件读写、产物管理、审批工作流；可选 Python `AgentRuntime`（`POST /agent/chat`） |
...
| Persistence | SQLite (WAL) via stdlib `sqlite3`；可选 PostgreSQL |
| API Server | Node.js 20 / @earendil-works/pi-coding-agent |
| Frontend | Vite 构建的 Vanilla JS SPA（`frontend/src/*.js`） |
| Agent SDK | Node: pi-coding-agent；可选 Python `AgentRuntime`（`AGENT_RUNTIME`） |
| MCP Adapter | Python `mcp` library |
| Container | Docker, docker compose |
| Testing | pytest；`node:test`（api-server + frontend）；CI 见 `.github/workflows/test.yml` |
