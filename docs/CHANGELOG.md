...
- **⌘K 命令面板**：在任何页面按 ⌘K 可同时搜索会话、产物与常用操作；侧栏「定时任务」在有新运行结果时显示小圆点。
...
- **管理端运行列表区分同一会话的多轮**：此前每行只显示会话标题，同一会话的几次运行看起来像重复记录；现在主文字是
  这一轮的用户输入，并标注「第 N 轮」，搜索也能匹配用户输入。运行详情页只在运行本身读不到时报「找不到」，时间线或
  工具台账读取失败时保留其余内容并提示。
- **会话标题栏始终显示所属智能体**：此前组织默认智能体被隐藏（误用了侧栏「默认不打标签」的规则）。
- **刷新后的会话按真实顺序还原**：会话历史回放此前先用 run 行的 `last_sequence` 设置游标，持久事件全部被
  判为重复而跳过，正文只能来自消息表，文字与工具的先后顺序丢失。现在刷新后与实时流走同一条事件回放。
- **思考不再串到上一轮**：新一轮的思考在没有流式消息时会追加到上一轮已完成的消息里。
- **批准或回答后结果实时出现**：等待审批 / 回答期间刷新过页面时，批准后没有事件流接到恢复的 run 上，
  结果要再刷新才可见；续连时游标也会越过尚未应用的事件。现在决定成功后重新接上事件流，游标只记录
  本地已应用的位置。只包含待审批工具调用的一轮也会显示审批卡。
- **未选模型时可以发送图片**：此前前端把「没选模型」当成「没有模型」，附带图片一律被拦下并只闪过一条
  英文提示；现在按模型目录的默认模型判断是否支持看图，不支持时在输入框里明确提示。
...
- **对话改为线性流**：一轮回复按事件顺序展示「思考 → 文字 → 工具 → 文字」，取代顶部的步骤树；只思考不出
  文字的中间轮次并入相邻工具组。子代理 / 委派、远程委派（A2A）、任务清单、提问、审批、后台任务、产物
  各有卡片，审批与提问在对话流内处理。见 [design/frontend-redesign.md](design/frontend-redesign.md)。
- **布局**：侧栏改为图标导航 + 搜索（⌘K 聚焦）+ 按日期分组的会话列表，会话标注所属智能体并可筛选；
  设置、管理控制台、主题切换、退出登录收进账户菜单。右侧 Inspector 改为默认关闭的「资料」抽屉
  （产物 / 文件 / 数据集 / 进程），会话内不再显示 Trace 与工具明细。
- **输入框**：运行中 Enter 排队追问、⌘Enter 立即改向（取代改向 / 追问切换按钮）；「＋」菜单可上传文件或
  引用其他会话的产物；待上传图片显示缩略图。
- **界面中文化与配色**：主界面文案改为中文；配色改为中性灰加钴蓝，只用系统字体。

### Changed（前端重设计第 2 期）

- **定时任务页重做**：「定时任务 / 运行」tab、最近 30 天运行条、⋯ 操作菜单、可跳转到运行产生的会话；
  新建 / 编辑改为频率构造器并实时预览接下来的触发时间。一次性任务的时间此前按浏览器时区解析、忽略了
  任务所选时区，现在按任务时区换算。
...
- **智能体页重做**：左侧智能体列表、右侧分页编辑器（基本信息 / 模型 / 工具权限 / MCP / 版本历史 / JSON），
  顶部固定显示编辑基于的版本、未保存与校验状态；工具权限按类别分组、改为四段选择并可只看已覆盖项；
  界面改为中文。保存仍然生成新版本，版本冲突处理不变。管理控制台此前被全局样式压成单列弹性布局，
  现在按左导航 + 内容区网格显示。
- **运行页按原型重做**：统计条、搜索与状态 / 智能体 / 时间筛选、整行可点的表格；点进独立的运行详情页
  （`/admin/runs/:runId`），以瀑布图展示排队、模型轮次、工具、子任务与审批等待，选中节点可看模型的思考与输出、
  工具参数与结果、子任务 prompt 与结论；另有工具台账与进程日志两个 tab。内容由持久事件与工具台账拼接，
  不改 trace 契约。旧的 Trace 树组件（`widgets/trace-panel`）移除。
- **审批 / A2A 接入页重做**：统一为管理控制台样式并改为中文。审批页改为紧凑卡片；A2A 页分
  「凭据 / 调用记录 / 审计 / 接入示例」。取消运行、吊销凭据改为页面内二次确认，不再弹浏览器对话框。
