...
### 2026-09-01 补做的两件事（原清单外）

- **Skill 启用面**：补齐 Agent HTTP → BFF → Capabilities UI；发布副本、owner-scoped
  MySQL 账本与 exec 逐包 `ro_bind` 同步收口。
- **并行工具停泊**：审批使 Run 停泊时，其它在飞工具统一记为 `UNKNOWN`，不留永久
  `RUNNING`。
...
- [x] **H9.6** ✅ **跑了，而且抓到 5 个单测全绿时仍存在的缺陷**。
      ...
      **之前那句「macOS 上没有 Bubblewrap」是错的**——`exec` 跑在 Linux 容器里，
      ...
      走同一条闸门代码）。2026-09-01 已另补 HTTP/BFF/UI 启用面。
