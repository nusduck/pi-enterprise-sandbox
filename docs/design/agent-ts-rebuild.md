...
> 下面 P1–P6 是 **2026-08-30 开工时**的现状。阶段 0 / A′–G 做完之后这些已经不成立
> （见顶部进度表）。保留原文是为了对照「当时为什么要分这些阶段」，不要当现在的缺口。
...
阶段表上的工作已经结束。下面这些**不是本方案漏做的阶段**，但接手的人常会
把它们当成「整理还没完」——列在这里，避免和 C–F 搞混。权威剩余项见
[waves/HANDOFF.md](waves/HANDOFF.md)。

- **`strict: false`**：已知待办。`src/runtime/**` 已单独保住 strict。
- **测试仍是 JS**：约 130 个 `agent/tests/**/*.js` 没迁。
- **`pi-` 文件名**：`pi-run-executor.ts` 等，纯改名。
- **真机 / CI / STATUS 重新取证**：不在本方案范围（不动 `exec/`、不动公共面契约）。
