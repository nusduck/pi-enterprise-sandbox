...
- **业务库是否强制 TLS**（未答复；一期按不强制实现）（如 MySQL `require_secure_transport`）？客户端经 unix socket 一般不发起 TLS；
  若强制，转发器需在出口一段做 TLS，§4.4 增加证书配置。
- **DBPM 口令是否轮换**？（未答复；一期启动时取一次，轮换后需重启 exec。）轮换则需定期重取，或在连接失败时重取。
