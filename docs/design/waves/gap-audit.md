...
> **2026-08-31：** 搜索 / 产物 / 数据集三块已在 Wave 7 按语义补齐（见各节顶部的
> 「已补」栏）。这份对照保留当时的 ❌ 表，方便核对「少了什么」；**不要把表里的
> ❌ 当成现在的缺口。** 现在还没移植的（Skill 根搜索、产物溯源）写在
> [HANDOFF.md](HANDOFF.md)。

写这份东西的直接原因：[README.md](README.md) 的进度表把 Wave 3 标成 ✅，
`exec` 的 231 个用例也全绿，但公共面的产物、数据集与搜索三块**当时是占位实现**。
