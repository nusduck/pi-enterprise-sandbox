...
2. `agent/runtime/agent-runtime.js` — Pi Session Host 与单轮运行编排；`agent/chat-runner.js` 仅为兼容导出
3. `agent/packages/enterprise-agent-kit/extensions/sandbox-tools/` — read/write/edit/bash/process/artifact 工具 Extension
4. `agent/packages/enterprise-agent-kit/extensions/skill-management/` — SKILLS_MODE 下的 install/edit/reload Extension
5. `agent/packages/enterprise-agent-kit/extensions/policy/` — 企业策略 Extension（含 Skill 根硬拒绝）
6. `agent/infrastructure/mcp-connection-manager.js` — Sandbox 之外的 MCP 连接、鉴权与 Schema 校验
7. 语法检查: `node --check agent/server.js && node --check agent/runtime/agent-runtime.js`
8. 单元测试: `npm test --prefix agent`
