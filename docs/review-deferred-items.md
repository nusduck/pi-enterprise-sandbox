...
| Pi's two tool-arg shapes can still collide on args integrity | `tool_execution_start` carries `toolCall.arguments` (raw) and creates the `policyPending` placeholder; the `tool_call` policy hook carries `validatedArgs` (`structuredClone` + TypeBox `Value.Convert`). `getOrCreate()` asserts replay match **before** overwriting `arguments_json`, so a model emitting e.g. `"2"` for an integer parameter yields two fingerprints and the same `args integrity` conflict the 512-truncation bug produced | Low frequency, high blast radius (Run FAILED) | One authoritative arg shape per ToolExecution | Not reproduced, so not speculatively fixed (AGENTS.md §3). If the conflict recurs with args **under** 512 chars, let the adopt branch overwrite the placeholder args with the policy's validated args — that branch is already guarded as side-effect-free |
