...
| Cancelling a parent Run does not cascade to its sub-agent children | `CancelRunService` cancels one `run_id`; children created by `spawn_subagent` carry `parent_run_id` but nothing walks it. Each child is individually cancellable through the ordinary cancel API, and the fan-out is bounded (`maxConcurrent` live children per parent, `maxDepth` deep), so a runaway tree is capped — but a cancelled parent leaves its children running to their own deadline | Medium for long child tasks (wasted sandbox/provider spend after the user gave up) | One cancel stops the whole tree | Walk `runs.parent_run_id` in `CancelRunService` (depth-bounded) and request the cancel signal for each non-terminal descendant; decide whether a *failed* parent should cascade too |
| Sub-agent conversations appear as top-level conversations | Each child Run gets its own Conversation + AgentSession (it must — the parent holds its own session's execution fence for its whole life), and `conversations.listForOwner` has no lineage filter, so a fan-out adds N rows to the owner's conversation list | Low (they are the owner's own runs, correctly scoped); UI noise only | Child transcripts grouped under the parent Run instead of listed beside it | Either a `parent_conversation_id`/lineage flag the list can filter, or a UI that renders children from `check_subagent` results; do not archive them at creation — that would make the transcript unreadable through the conversation API |
