...
本 ADR 锁定迁移到这三者的架构决策。部分语法与场景有历史真机探针证据，
证据与推导过程见原改造方案 §9；不代表并发、整条链路或目标部署已验收。

### 2026-09-12 实施细化与证据边界

后续实施以 [统一 design](../design/updrdb-dbpm-deployment.md) 为入口；本 ADR 的迁移方向保留，未实施状态不变。

- D3：旧探针先提交 A 再执行 B，只证明条件更新；真正重叠事务尚待验证。Outbox 保留发布 CAS；Cron 新增事务内 token，保留调度策略、执行记录与时间推进的原子事务，见 design §5。
- D5/D7：`contract/` 只共享纯选择器；驱动适配留在各基础设施层。函数式 Knex connection 不足以捕获建连失败；UTC 初始化必须等待成功，见 §4。
- D6：不能把不完整的 `schema-tables.ts` 当作完整 schema。按迁移导出 manifest，Agent、Worker、exec 均在服务/消费/恢复前验证列、索引、约束和触发器，见 §6。
- D2/D9：5.7 必须用新数据卷；队列改 prefix 必须停准入、drain 和核对恢复，见 §8/§10。保留旧数据，不进行原地降级。
- 拓扑：共享 Skill 存储可提供；入口 HTTPS 是设计要求，资源待确认；发布一致性、VM 隔离与真实链路仍是上线门槛，见 §3/§12。

下文保留原决策上下文；上述细化修正过宽的实现/证据表述，不将设计完成记为验收通过。
...
`claim_token` 回读。历史顺序探针中 A 得 1 行、B 得 0 行；真正并发互斥须按新设计验证。
