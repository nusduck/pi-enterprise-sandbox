 ...
 *
 * 建连走 `failover-pool.ts`：UPDRDB 给两个 Proxy（`UPDRDB_ENDPOINTS`），按端点
...
import {
  ...
  type ExecDbPool,
} from './failover-pool.js';

export { SESSION_UTC_SQL };
export type { ExecDbPool };
...
export interface ExecDbConfig {
  ...
  /** 可选：UPDRDB 两个 Proxy。给了就覆盖 host/port；不给就用 host/port 单端点。 */
 ...
 * 否则连接数会按仓储数线性放大，且关闭时漏关某一个池会悬空连接，也会绕开
 * 端点故障切换和会话初始化。
 *
 ...
 * 这里是第一道防线。
 */
export function createExecDbPool(
  config: ExecDbConfig,
  deps: { readonly driver?: EndpointPoolDriver | undefined } = {},
): ExecDbPool {
  const pool = createFailoverPool({
    ...
    endpoints: config.endpoints ?? [{ host: config.host, port: config.port }],
    role: 'exec-db',
    driver: deps.driver,
  });

  // `execute` 的错误脱敏（轻量 AOP，不改驱动行为）。
  const originalExecute = pool.execute as unknown as (sql: unknown, params?: unknown) => Promise<unknown>;
  const execute = (async (sql: unknown, params?: unknown) => {
    try {
...
/** 关闭池。幂等：重复调用不抛。 */
export async function closeExecDbPool(pool: ExecDbPool): Promise<void> {
  try {
    await pool.end();
  } catch (err: unknown) {
